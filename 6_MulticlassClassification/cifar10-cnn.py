...
model = Sequential([
    ...
    data_augmentation,
    layers.Conv2D(64, kernel_size = (3, 3), padding='same', activation = None),
    layers.BatchNormalization(),
    layers.Activation('relu'),
    layers.MaxPooling2D(pool_size = (2, 2), strides = 2),
    layers.Conv2D(128, kernel_size = (3, 3), padding='same', activation = None),
    layers.BatchNormalization(),
    layers.Activation('relu'),
    layers.MaxPooling2D(pool_size = (2, 2), strides = 2),
...
scheduled_lr = optimizers.schedules.CosineDecay(initial_learning_rate=0.05, decay_steps = len(x_train) // 128 * 50)
optimizer = optimizers.SGD(learning_rate=scheduled_lr, momentum=0.9, nesterov=True)
